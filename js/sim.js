...
export class Simulator {
  ...
  tick() {
    ...
    const pxps = this.state.mps / this.state.m_per_px; // pixels per second
    const bounds = { w: this.state.site.width_m, h: this.state.site.height_m };

    for (const a of this.state.assets) {
      // velocity
      let [vx, vy] = normalize(a.vel[0], a.vel[1]);
      vx *= pxps; vy *= pxps;

      // predicted next position (meters)
      const nx = clamp(a.pos[0] + (vx * dt) * this.state.m_per_px, 0, bounds.w);
      const ny = clamp(a.pos[1] + (vy * dt) * this.state.m_per_px, 0, bounds.h);

      // zone enter/exit check
      const prevInside = new Set(a.insideZones || []);
      const currentInside = new Set();
      for (const z of this.state.zones.zones) {
        const inside = this.state.pointInZone([nx, ny], z);
        if (inside) {
          currentInside.add(z.id);
          if (!prevInside.has(z.id)) this.state.emitEvent('ENTER_ZONE', { asset:a.id, zone:z.name });
        } else {
          if (prevInside.has(z.id)) this.state.emitEvent('EXIT_ZONE', { asset:a.id, zone:z.name });
        }
        if (inside && z.type==='no_go') {
          // simple bounce by flipping direction
          a.vel[0]*=-1; a.vel[1]*=-1;
        }
      }
      a.insideZones = Array.from(currentInside);

      // proximity (MEET/LEAVE)
      for (const b of this.state.assets) if (b!==a) {
        const d = dist(a.pos, b.pos);
        const threshold = (a.bubble_m||3) + (b.bubble_m||3);
        const k = a.id + '|' + b.id;
        a._meet = a._meet || {};
        const prev = !!a._meet[k];
        const nowMeet = d <= threshold;
      ...
      // apply move
      const moved = (Math.hypot(nx - a.pos[0], ny - a.pos[1]) > 1e-3);
      if (moved) a.lastMovedAt = t;
      a.pos[0] = nx; a.pos[1] = ny;

      // idle -> gray after 5 min
      a.status = (t - (a.lastMovedAt || t)) > 5*60*1000 ? 'gray' : (a.statusBase || 'green');

      // trails (store every ~250ms)
      a._trailAcc = (a._trailAcc || 0) + dt;
      if (a._trailAcc > 0.25) {
        a._trailAcc = 0;
        const arr = this.trails.get(a.id) || [];
        arr.push({ t, x:a.pos[0], y:a.pos[1] });
        // drop older than historyMinutes
        const cutoff = t - this.historyMinutes*60*1000;
